...
export class UsersService {
  ...
  insertNewUser(user: User): Observable<any> {
    return this.http.post<any>(`${this.urlApi}`, JSON.stringify(user), this.httpOptions);
  }

  updateUser(username: string, user: User): Observable<any> {
    return this.http.put<any>(`${this.urlApi}/${username}`, JSON.stringify(user), this.httpOptions);
  }

  deleteUser(username: string): Observable<any> {
    return this.http.delete(`${this.urlApi}/${username}`);
  }
}
